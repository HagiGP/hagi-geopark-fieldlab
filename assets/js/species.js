...
(function(){
  ...
  function open(card){
    ...
    // data-count / data-rounds は複数回の調査を載せるラボだけが持つ（無いカードとの後方互換）
    var countRow=d.count ? '<dt>採集数</dt><dd>'+d.count+'</dd>' : '';
    var roundRow=d.rounds
      ? '<dt>確認した回</dt><dd>'+d.rounds.split('・').join('／')+'</dd>'
